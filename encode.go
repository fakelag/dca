...
type EncodeOptions struct {
	Volume           float64          // change audio volume (1.0=normal)
...
func (e *EncodeSession) run() {
	...
	if e.options.Volume != 1.0 {
		args = append(args, "-filter:a", fmt.Sprintf("volume=%.2f", e.options.Volume))
	}
...
func (e *EncodeSession) handleStderrLine(line string) {
	...
	_, err := fmt.Sscanf(line, "size=%dKiB time=%d:%d:%f bitrate=%fkbits/s speed=%fx", &size, &timeH, &timeM, &timeS, &bitrate, &speed)
	if err != nil {
		logln("Error parsing ffmpeg stats:", err, line)
	}
